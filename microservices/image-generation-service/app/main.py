"""
Main application module for the image generation service.
Implements FastAPI app, RabbitMQ consumer, and job processing logic with Cloudinary integration.
Handles text-to-image generation with various aspect ratios and quality levels.
"""
...
from app.processing import perform_image_generation, ImageProcessingError
...
app = FastAPI(
    ...
    description="Microservice for AI-powered text-to-image generation using Stable Diffusion with Cloudinary integration",
    version="0.1.0",
)
...
async def startup_event():
    ...
    try:
        ...
        logger.info("Image Generation Service with Cloudinary started successfully")
...
async def process_message(message: AbstractIncomingMessage) -> None:
    ...
    while retry_count <= MAX_RETRIES:
        try:
            ...
            # Perform image generation with Cloudinary integration
            processed_image_url, processing_params = await perform_image_generation(
                job_id,
                job_dto.jobConfig or {}  # No input image needed for generation
            )
...
async def start_rabbitmq_consumer() -> None:
    ...
    while retries < max_retries:
        try:
            ...
            logger.info("Cloudinary integration enabled for text-to-image generation processing")
